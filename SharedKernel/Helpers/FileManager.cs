...
namespace SharedKernel.Helpers
{
    ...
    {
        /// <summary>
        /// Extract zip file into a path
        /// </summary>
        /// <param name="path">The directory to create</param>
        public static bool ExtractZipToDirectory(string archivefile, string path)
        {
            if (archivefile == null || archivefile.Length == 0 || FileManager.GetFileExtension(archivefile) != ".zip") return false;
            DeleteDirectory(path);
            CreateDirectory(path);
            //unzip archive
            ZipFile.ExtractToDirectory(archivefile, path);
            return true;
        }
        ...
        public static void FileCopy(string sourceFileName, string destFileName, bool overwrite = false)
        {
        ...
        /// </returns>
        public static string[] GetFiles(string directoryPath, string searchPattern = "", bool topDirectoryOnly = true)
        {
            if (string.IsNullOrEmpty(searchPattern))
                searchPattern = "*.*";
        ...
        public static string GetVirtualPath(string root, string path)
        {
            if (string.IsNullOrEmpty(path))
                return path;
